...
class _QuantityControllerButton extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final menuItemId = rec['menuItemId'] as String?;
    final branchId = rec['branchId'] as String? ?? '';
    final name = rec['dishName'] as String? ?? '';
    final price = (rec['priceAmount'] as num?)?.toDouble() ?? 0.0;

    if (menuItemId == null) return const SizedBox.shrink();
